...
def get_model_prediction(model, x):
    """从模型输出中获取预测结果，始终返回一维 numpy 数组（batch 长度）。"""
...
def sim_attack(x, y, model, eps=16/255, iterations=10, mu=1.0):
    """Scale-Invariant + Momentum Iterative FGSM (SIM-MI-FGSM)，支持批量。"""
    device = x.device
    x_adv = x.clone().detach()
    alpha = eps / max(iterations, 1)
    momentum = torch.zeros_like(x_adv, device=device)

    # 多尺度列表（缩放不变性）：直接按系数缩放输入张量
...
def parse_args():
    parser = argparse.ArgumentParser(description='SIM-MI-FGSM attack (batch + CLI)')
    parser.add_argument('--model', default='tf2torch_inc_res_v2', type=str, help='source model')
    ...
    parser.add_argument('--output_csv', default='./results/sim/attack_results_SIM.csv', type=str, help='output CSV path')
...
def main_cli():
    ...
    results = []
    print("\nStarting SIM-MI-FGSM Attack...")
    for x_batch, y_batch, filename_batch in tqdm(loader, desc="Running SIM"):
        x_batch = x_batch.to(device)
        y_batch = y_batch.to(device)

        # 源预测与对抗样本
        source_orig_preds = get_model_prediction(source_model, x_batch)
        x_adv_batch = sim_attack(x_batch, y_batch, source_model, **attack_params)
    ...
    # 输出统计并保存 CSV
    print("\n" + "="*80)
    print(f"Summary of SIM-MI-FGSM Attack Results (Source: {args.model})")
    print("="*80)
